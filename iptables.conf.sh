...
iptables -A INPUT -p tcp -s 195.165.26.0/25 -d 195.165.26.0/25 --dport 1313 -m state --state NEW,ESTABLISHED,RELATED -j accept-input
iptables -A OUTPUT -p tcp -s 195.165.26.0/25 -d 195.165.26.0/25 --sport 1313 -m state --state ESTABLISHED,RELATED -j accept-output
...
iptables -A INPUT -p udp -s 195.165.26.0/25 -d 195.165.26.0/25 --dport 53  -j accept-forward
iptables -A FORWARD -p udp -s 195.165.26.0/25 -d 195.165.26.0/25 --sport 53  -j accept-forward

#Allow DHCP traffic between the Windows Server and client
